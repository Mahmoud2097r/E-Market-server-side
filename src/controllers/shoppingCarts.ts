...
export const postShoppingCart = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { product_id, shoppingCart_id } = req.body
        if (product_id == null || product_id === '') throw new Error()
        ...
        if (shoppingCart_id === '' || shoppingCart_id == null) {
            const shoppingCart = await new ShoppingCart().populate({
                path: 'products',
            })
        ...
        const shoppingCart = await ShoppingCart.findById(shoppingCart_id).populate({
            path: 'cartItems',
        })
        ...
        next(new ExpressError('Sorry, Something went wrong try again later!', 404))
    }
}

export const updateShoppingCart = async (req: Request, res: Response, next: NextFunction) => {
    try {
